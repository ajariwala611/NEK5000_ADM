...
def view1(cam):
    cam.SetPosition(3.75,7.5,0.75)     # Camera position
    cam.SetFocalPoint(3.75,0.75,0.75)    # Camera focal point
    # cam.Elevation(0)
    # cam.Yaw(0)
    # cam.Roll(0)
    cam.Zoom(1.0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iso_u', action='store_true', default=False)
    parser.add_argument('--parallel', action='store_true', default=False)  
    parser.add_argument('--slice', action='store_true', default=False)
    parser.add_argument('--streamlines', action='store_true', default=False)
...
#   reader =  OpenDataFile('/Volumes/ResearchDat/nek5000/Nek5000/run/turbulent_flow_control/turbulent_boundary_layer/statistics/flat_plate.nek5000')
    ...
    if args.iso_u:
        ...
        u = 0.0    # Change u isosurface value here
    ...
    if args.clip:
        ...
        height = 1.0    # Change height here

        clip = Clip(reader)
        clip.ClipType.Normal = [0,0,1]
        clip.ClipType.Origin = [15,1,height]

        # Clip Coloring
        color_by = 'x_velocity'
        #color_range = [-0.01, 1.0]
    ...
    if args.slice:
        ...
        #gradient1 = Gradient(reader)
        #gradient1.ScalarArray = 'velocity'
        slice = Slice(reader)
        slice.SliceType = 'Plane'
        slice.SliceType.Normal = [0,1,0]
        slice.SliceType.Origin = [3.75,0.75,0.75]
        ...
        dp.Representation = 'Surface'
        dp.LookupTable = GetColorTransferFunction(color_by)
        dp.ColorArrayName = 'x_velocity'
        #dp.SetScalarBarVisibility(view, True)
        #dp.LookupTable.ApplyPreset('Linear Green (Gr4L)', True)

        slicedisp = Show(slice)
        slicedisp.SetScalarBarVisibility(view, True)
        #ColorBy(slicedisp, ('POINTS', color_by, '1'))
    ...
    if args.q_criterion:
        ...
        q_grid_initial.WholeExtent = [0, 3000, 0, 200, 0, 750]
        ...
        dp.LookupTable = GetColorTransferFunction(color_by)
        dp.ColorArrayName = color_by
        dp.LookupTable.RescaleTransferFunction(-0.05, 1.0)
        qdisp = Show(contour)
        qdisp.SetScalarBarVisibility(view, True)
        #ColorBy(qdisp, ('POINTS', color_by))

    
    if args.streamlines:

        streamlines = StreamTracer(q_grid_data)
        streamlines.SeedType = 'Line'
        streamlines.Vectors = ['POINTS','velocity']
        
        streamlines.MaximumStreamlineLength = 30.0            
        streamlines.SeedType.Point1 = [7.0, 0.0, 1.0]
        streamlines.SeedType.Point2 = [7.0, 0.4, 1.0]
        streamlines.SeedType.Resolution = 10

        color_by = 'none'
        dp = GetDisplayProperties(streamlines)
        dp.Representation = 'Surface'
        dp.LookupTable = GetColorTransferFunction(color_by)
