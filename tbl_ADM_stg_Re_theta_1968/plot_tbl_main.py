...
def view1(cam):
    cam.SetPosition(15,27.5,1.25)     # Camera position
    cam.SetFocalPoint(15,0.5,1.25)    # Camera focal point
    cam.SetViewUp(0,0,-1)
    # cam.Elevation(0)
    # cam.Yaw(0)
    # cam.Roll(0)
...
#   reader =  OpenDataFile('/Volumes/ResearchDat/nek5000/Nek5000/run/turbulent_flow_control/turbulent_boundary_layer/statistics/flat_plate.nek5000')
    ...
    if args.slice:
        ...
        slice.SliceType.Origin = [15,0.5,1.25]

        # Slice Coloring
        color_by = 's1'
        color_range = [-0.5, 0.3]

        arrayInfo = slice.PointData[color_by]
        AssignLookupTable(arrayInfo, "Cool to Warm", rangeOveride=color_range)
